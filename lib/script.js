...
exports.jsParser = function(file, cb, options) {
  ...
  var metaId = meta.id ? meta.id : unixy(options.idleading + filename.replace(/\.js$/, ''));
  var reg = options.prefixRegex  || /^build\//, prefixReplace = options.prefixReplace || "";;
  if(options.delFirstLetterOfbuild && metaId && metaId.match(reg)){
    metaId = metaId.replace(reg, prefixReplace);
  }
